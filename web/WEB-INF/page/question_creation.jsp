...
<head>
    ...
    <fmt:message bundle="${loc}" key="local.question.topic.label" var="topicLabel"/>
    <fmt:message bundle="${loc}" key="local.question.topic.placeholder" var="topicPlaceholder"/>
    <fmt:message bundle="${loc}" key="local.question.content.label" var="contentLabel"/>
    <fmt:message bundle="${loc}" key="local.question.content.placeholder" var="contentPlaceholder"/>
    <fmt:message bundle="${loc}" key="local.question.askbutton" var="askBtn"/>
    <fmt:message bundle="${loc}" key="local.question.deletebutton" var="deleteBtn"/>
    <fmt:message bundle="${loc}" key="local.question.editbutton" var="editBtn"/>
</head>
<body>
                    ...
                    <c:if test="${requestScope.question != null}">
                        ...
                        <div class="form-row">
                            <div class="col-12 mb-3">
                                <label for="topic_editing"><c:out value="${topicLabel}" /></label>
                        ...
                        <div class="form-row">
                            <div class="col-12 mb-3">
                                <label for="content_editing"><c:out value="${contentLabel}" /></label>
                        ...
                        <input class="btn btn-info" type="submit" value="${editBtn}"/>
                    </c:if>
                    <c:if test="${requestScope.question == null}">
                        <input type="hidden" name="command" value="question_creation"/>
                        <div class="form-row">
                            <div class="col-12 mb-3">
                                <label for="topic"><c:out value="${topicLabel}" /></label>
                        ...
                        <div class="form-row">
                            <div class="col-12 mb-3">
                                <label for="content"><c:out value="${contentLabel}" /></label>
                                <textarea class="form-control" id="content" name="content"
                                          placeholder="${contentPlaceholder}" minlength="50" required></textarea>
                            </div>
                        </div>
                        <input type="submit" class="btn btn-info" value="${askBtn}"/>
                    </c:if>
                </form>
            </div>
        </div>
